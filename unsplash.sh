if [ ! -d "/home/$USER/.unsplash" ]; then
  mkdir -p /home/$USER/.unsplash
  touch /home/$USER/.unsplash/.theme
fi

readonly PIC_PATH="/home/$USER/.unsplash"
...
if [ -z $1 ] || [ $1 == "new-bg" ]; then
...
elif [ $1 == "list-favs" ]; then
	favfolds=$(ls -d $PIC_PATH/*/ | xargs -n 1 basename)
  while read -r line; do
    echo "$line ... "$(ls -1 "$PIC_PATH/$line/"| wc -l)""
  done <<< "$favfolds"
else
  if [ ! $1 == "help" ]; then
  ...
  echo "- SAMpaper list-favs                    # shows you what favourites you have, and how full they are"
